...
NTSTATUS ProxIoctlFuntion(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
	...
	case CommandMemoryRead:
	{
		...
		Status = ReadProcessMemory(mrd->Pid, mrd->VirtualAddress, mrd->Buffer, mrd->BufferSize, mrd->NumberOfBytes);
	...
	{
		PMEMORY_RW_DESC mrd = (PMEMORY_RW_DESC)pIrp->AssociatedIrp.SystemBuffer;
		Status = WriteProcessMemory(mrd->Pid, mrd->VirtualAddress, mrd->Buffer, mrd->BufferSize, mrd->NumberOfBytes);
		if (NT_SUCCESS(Status))
		{
			mrd->Status = TRUE;
		}
	...
	case CommandHideFromSyscall:
	{
		...
		//if (!HookSyscalls())
		//{
		//	ShvOsDebugPrint("One Key Hook Faild!\n");
		//	pIrp->IoStatus.Status = STATUS_UNSUCCESSFUL;
		//	pIrp->IoStatus.Information = 0;
		//	break;
		//}
		////第二步，添加目标pid
		//PHIDE_INFO HideInfo = (PHIDE_INFO)pIrp->AssociatedIrp.SystemBuffer;
		//ULONG pid = HideInfo->Pid;
		//ShvOsDebugPrint("CommandAddHiderEntry pid:[%d]\n", pid);
		//if (CreateEntry(IoGetCurrentProcess(), PidToProcess((HANDLE)pid)) == FALSE)
		//{
		//	pIrp->IoStatus.Status = STATUS_UNSUCCESSFUL;
		//	pIrp->IoStatus.Information = 0;
		//	break;
...
BOOLEAN InitOffsets()
{
	...
	if (NT_SUCCESS(Status))
	{
		...
		ShvOsDebugPrint("LdrLoadDll:[%p]\n", GlobalConfig::Instance().LdrLoadDll);
	}
...
EXTERN_C NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pRegPath)
{
	...
	do
	{
		...
		//隐藏
		//if (HiderInitialize() == FALSE)
		//{
		//	ShvOsDebugPrint("HiderInitialize Faild!\n");
		//	break;
		//}

		//if (NT_SUCCESS(PsSetCreateThreadNotifyRoutine(ThreadNotifyRoutine)) == FALSE)
		//{
		//	ShvOsDebugPrint("PsSetCreateThreadNotifyRoutine Faild!\n");
		//	HiderUninitialize();
		//	break;
		//}
		//ShvOsDebugPrint("PsSetCreateThreadNotifyRoutine succeded\n");

		//if (NT_SUCCESS(PsSetCreateProcessNotifyRoutine(ProcessNotifyRoutine, FALSE)) == FALSE)
		//{
		//	ShvOsDebugPrint("PsSetCreateProcessNotifyRoutine Faild!\n");
		//	HiderUninitialize();
		//	PsRemoveCreateThreadNotifyRoutine(ThreadNotifyRoutine);
		//	break;
		//}
		//ShvOsDebugPrint("PsSetCreateProcessNotifyRoutine succeded\n");
		...
		//if (EptHookDriverCallBackInit() == FALSE)
		//{
		//	ShvOsDebugPrint("EptHookDriverCallBackInit Faild!\n");
		//	break;
		//}
